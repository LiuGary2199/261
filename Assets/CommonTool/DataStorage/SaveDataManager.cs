...
public class SaveDataManager
{
    ...
    public static void SetBoolArray(string key, bool[] value)
    {
        ...
        {
            PlayerPrefs.SetString(key + "BoolArray" + i, value[i].ToString());
        }
        PlayerPrefs.SetInt(key + "BoolArray", value.Length);
    }

    public static bool[] GetBoolArray(string key)
    {
        bool[] boolArr = new bool[0];
        if (PlayerPrefs.GetInt(key + "BoolArray") != 0)
        {
            boolArr = new bool[PlayerPrefs.GetInt(key + "BoolArray")];
            for (int i = 0; i < boolArr.Length; i++)
            {
                boolArr[i] = bool.Parse(PlayerPrefs.GetString(key + "BoolArray" + i));
            }
            return boolArr;
        }
        return boolArr;
