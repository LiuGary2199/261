...
public class BaseUIForms : MonoBehaviour
{
    ...
    protected virtual void Awake()
    {
        ...
        string[] ClostBtnPath = { "关闭按钮", "ClostBtn", "ClostButton", "Window/Content/CloseBtn" };
        foreach (string path in ClostBtnPath)
        {
            if (transform.Find(path))
            {
                close_button = transform.Find(path).GetComponent<Button>();
                if (close_button != null)
                {
                    close_button.onClick.AddListener(() => { UIManager.GetInstance().CloseOrReturnUIForms(this.GetType().Name); });
                    break;
                }
            }
        }
    ...
    public virtual void Display(object uiFormParams)
    {
        ...
        if (_CurrentUIType.UIForm_animationType == UIFormShowAnimationType.fade) //淡入
        {
        ...
        Transform Title = transform.Find("标题");
        if (Title != null)
        {
            Title.DOKill();
            Title.localScale = new Vector2(0, 1);
            Title.DOScaleX(1, 0.3f).SetEase(Ease.OutBack).SetDelay(0.3f);
            Transform titleText = Title.GetChild(0);
            if (titleText != null)
            {
                titleText.DOKill();
                titleText.localScale = new Vector2(1, 0);
                titleText.DOScaleY(1, 0.3f).SetEase(Ease.OutBack).SetDelay(0.6f);
            }
        }
    }
    //页面隐藏（不在栈集合中）
    public virtual void Hidding(System.Action finish = null)
    {
        ...
        {
            if (_CurrentUIType.UIForm_animationType == UIFormShowAnimationType.fade) //淡出
            {
                CanvasGroup canvasGroup = gameObject.GetComponent<CanvasGroup>();
    ...
    protected BaseUIForms OpenUIForm(string uiFormName)
    {
        return UIManager.GetInstance().ShowUIForms(uiFormName).GetComponent<BaseUIForms>();
    }
