...
public class UIManager : MonoBehaviour
{
    ...
    //弹出节点
    [HideInInspector] public Transform _TraPopUp = null;
    ...
    private void InitCamera()
    {
        ...
        if (currentPipeline != null && currentPipeline.name == "UniversalRenderPipelineAsset")
        {
            ...
            UICamera = MainCanvas.GetComponentInChildren<Camera>();
    }
    ...
    /// <returns></returns>
    public BaseUIForms GetPanelByName(string uiFormName)
    {
        //ui窗体基类
        BaseUIForms baseUIForm;
        //如果正在显示的集合中存在该窗体，直接返回
    ...
    public T GetPanelByType<T>() where T : BaseUIForms
    {
        //ui窗体基类
        BaseUIForms baseUIForm;
        //如果正在显示的集合中存在该窗体，直接返回
        ...
        _DicALLUIForms.TryGetValue(uiFormName, out baseUIForm);
        if (baseUIForm != null && baseUIForm is T)
    ...
    public BaseUIForms GetTopPanel() //获得最上层的打开的面板
    {
        BaseUIForms TopPanel = null;
        List<BaseUIForms> allUIFormsList = new List<BaseUIForms>(_DicALLUIForms.Values);
        foreach (BaseUIForms panel in allUIFormsList)
        {
            if (panel.gameObject.activeInHierarchy)
            {
                //UnityHelper.Print("打开的面板 ：" + panel.name);
                TopPanel = panel;
            }
        }
        //UnityHelper.Print("最上层面板：  " + TopPanel.name,1);
        return TopPanel;
    }

    public bool IsGamePanelTop() //判断游戏面板是否在最上层
    {
        BaseUIForms TopPanel = GetTopPanel();
        if (TopPanel.name != "MachinePanel")
            return false;
        return true;
    }
}
