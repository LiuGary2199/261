...
public class MusicMgr : MonoSingleton<MusicMgr>
{
    ...
    private void Start()
    {
        StartCoroutine("CheckUnUseAudioComponent");
    }
    /// <summary>
    /// 定时检查没有使用的音频组件并回收
    /// </summary>
    ...
    private void StopEffectBase(object effectName)
    {
        string effectNameStr = AudioSettingDict[effectName.ToString()].filePath.Split('/')[2];
        for (int i = 0; i < PlayAudioSourceList.Count; i++)
        {
            if (PlayAudioSourceList[i] != null && PlayAudioSourceList[i].clip != null && PlayAudioSourceList[i].clip.name == effectNameStr)
            {
                PlayAudioSourceList[i].Stop();
            }
        }
    }
    public void StopEffect(MusicType.UIMusic effectName)
    {
        StopEffectBase(effectName);
    }


    public void PlayToneUp()
    {
        if (_toneIndex > 15)
        {
            _toneIndex = 1;
        }
        PlayEffect((MusicType.UIMusic)_toneIndex);
        _toneIndex++;
        StopCoroutine(nameof(StopUseTone));
        StartCoroutine(nameof(StopUseTone));
    }

    IEnumerator StopUseTone()
    {
